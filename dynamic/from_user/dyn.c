...
static int my_open(struct inode *inode, struct file *file){
        printk(KERN_INFO"sridyn : device open\n");
        return 0;
}

static int my_release(struct inode *inode, struct file *file){
        printk(KERN_INFO"sridyn: device closed\n");
        return 0;
}

static ssize_t my_read(struct file *file, char __user *buf, size_t len, loff_t *ppos){
        printk(KERN_INFO"sridyn : read request\n");
...
static ssize_t my_write(struct file *file, const char __user *buf,size_t len, loff_t *ppos){
        printk(KERN_INFO"sridyn: write request\n");
        if(copy_from_user(kernel_buffer,buf,len))
                return -EFAULT;
        printk(KERN_INFO"user mgs:%s\n",kernel_buffer);
        return len;
}
